...
public final class STStore extends StoreBase implements ConfigurableStore<STStoreConfig> {
	...
	public STDocument get(GlobalOid<STDocument> oid) {
		return root.document(oid);
	}

	public STDocument get(String oid) {
		String namespace = "com.sandpolis.core.instance";

		STDocument document = root;
		for (var component : oid.replace("^/", "").split("/")) {
